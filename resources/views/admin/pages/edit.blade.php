...
@section('title', 'Edit Page — Page Builder — Debesties Studio')
...
<form id="page-form" action="{{ route('admin.pages.update', $page->id) }}" method="POST">
    ...
    <div id="panel-info">
        <div style="background: var(--cms-surface); border: 1px solid var(--cms-border); border-radius: var(--cms-r-xl); padding: 28px; box-shadow: var(--cms-sh-card); max-width: 800px; margin: 0 auto; display: flex; flex-direction: column; gap: 20px;">
            ...
            <div>
                ...
                <input type="text" name="title" id="page-title" value="{{ old('title', $page->title) }}" placeholder="e.g. About Us" required
            ...
            {{-- Slug info --}}
            <div style="background: var(--cms-bg); border-radius: var(--cms-r-md); padding: 12px 14px; font-family: var(--cms-font-ui), sans-serif; font-size: 12.5px; color: var(--cms-fg3);">
                <strong>Slug:</strong> <code style="color: var(--cms-fg2);">/{{ $page->slug }}</code>
                &nbsp;&middot;&nbsp; <strong>Created:</strong> {{ $page->created_at->format('M d, Y') }}
            </div>
        </div>
    </div>

    {{-- TAB 2: Visual Builder (same as create) --}}
        ...
        <div style="display: grid; grid-template-columns: 240px 1fr 300px; gap: 20px; align-items: start;">
            ...
            <div style="background: var(--cms-surface); border: 1px solid var(--cms-border); border-radius: var(--cms-r-xl); padding: 18px; box-shadow: var(--cms-sh-card); position: sticky; top: 88px;">
                <h3 style="font-family: var(--cms-font-ui), sans-serif; font-size: 12.5px; font-weight: 700; color: var(--cms-fg2); text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 10px; display: flex; align-items: center; gap: 6px;">
                    <i data-lucide="box" style="width: 14px; height: 14px; color: var(--cms-gold);"></i> Widgets
                </h3>
                <div style="display: flex; flex-direction: column; gap: 6px;" id="widget-palette"></div>

                <div style="border-top: 1px solid var(--cms-border); margin-top: 14px; padding-top: 14px;">
                    <h3 style="font-family: var(--cms-font-ui), sans-serif; font-size: 12.5px; font-weight: 700; color: var(--cms-fg2); text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 10px; display: flex; align-items: center; gap: 6px;">
                        <i data-lucide="columns" style="width: 14px; height: 14px; color: var(--cms-gold);"></i> Add Section
                    </h3>
                    <div style="display: flex; flex-direction: column; gap: 6px;" id="section-palette"></div>
                </div>
            </div>
            ...
            <div>
                <div style="background: var(--cms-surface); border: 1px solid var(--cms-border); border-radius: var(--cms-r-xl); padding: 20px; box-shadow: var(--cms-sh-card);">
                    ...
                    <div id="canvas-container" style="display: flex; flex-direction: column; gap: 12px; min-height: 300px; padding: 4px; border-radius: var(--cms-r-lg);"></div>
                </div>
            </div>
            ...
            <div style="background: var(--cms-surface); border: 1px solid var(--cms-border); border-radius: var(--cms-r-xl); padding: 18px; box-shadow: var(--cms-sh-card); position: sticky; top: 88px;">
                <h3 style="font-family: var(--cms-font-ui), sans-serif; font-size: 12.5px; font-weight: 700; color: var(--cms-fg2); text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 6px; display: flex; align-items: center; gap: 6px;">
                    <i data-lucide="settings" style="width: 14px; height: 14px; color: var(--cms-fg3);"></i> Inspector
                </h3>
                ...
                <div id="inspector-panel" style="display: none; margin-top: 12px;">
                    ...
                    <div id="inspector-fields" style="display: flex; flex-direction: column; gap: 12px; max-height: 50vh; overflow-y: auto; padding-right: 4px;"></div>
    ...
    <div style="position: sticky; bottom: 0; left: 0; right: 0; z-index: 50; background: rgba(244,241,236,0.92); backdrop-filter: blur(12px); border-top: 1px solid var(--cms-border); padding: 14px 24px; margin: 24px -24px -24px -24px; display: flex; justify-content: flex-end; gap: 10px;">
        <a href="{{ route('admin.pages.index') }}"
           ...
           onmouseover="this.style.background='var(--cms-bg)';" onmouseout="this.style.background='var(--cms-surface)';">Cancel</a>
...
<script id="page-layout-data" type="application/json">
    {!! json_encode($page->layout ?? []) !!}
</script>

<script>
    // ─── LOAD EXISTING LAYOUT ────────────────────────────────
    let sections = JSON.parse(document.getElementById('page-layout-data').textContent);
    // Ensure sections is an array
    if (!Array.isArray(sections)) sections = [];

    let nextId = 1;
    // Derive next ID from existing data
    function deriveMaxId(data) {
        let max = 0;
        if (Array.isArray(data)) {
            data.forEach(item => {
                if (item.id && item.id > max) max = item.id;
                if (item.columns) item.columns.forEach(col => {
                    if (col.id && col.id > max) max = col.id;
                    if (col.widgets) col.widgets.forEach(w => { if (w.id && w.id > max) max = w.id; });
                });
            });
        }
        return max;
    }
    nextId = deriveMaxId(sections) + 1;
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // If layout exists, auto-switch to builder tab
        if (sections.length > 0) {
            switchTab('builder');
        }
    });

    // ─── Identical JS functions as create view ───────────────
    function switchTab(tab) {
        ...
        if (tab === 'info') {
            infoPanel.style.display = 'block'; buildPanel.style.display = 'none';
            tabInfo.style.borderBottomColor = 'var(--cms-gold)'; tabInfo.style.color = 'var(--cms-fg1)';
            tabBuilder.style.borderBottomColor = 'transparent'; tabBuilder.style.color = 'var(--cms-fg3)';
        } else {
            infoPanel.style.display = 'none'; buildPanel.style.display = 'block';
            tabBuilder.style.borderBottomColor = 'var(--cms-gold)'; tabBuilder.style.color = 'var(--cms-fg1)';
            tabInfo.style.borderBottomColor = 'transparent'; tabInfo.style.color = 'var(--cms-fg3)';
        }
    }
    ...
    function addSection(cols) {
        const columns = [];
        for (let i = 0; i < cols; i++) columns.push({ id: nextId++, widgets: [] });
        sections.push({ id: nextId++, background: 'white', padding: 'medium', columns });
        renderCanvas();
        showToast(`${cols}-column section added.`);
    }

    function promptAddWidget(widgetType) {
        if (sections.length === 0) addSection(1);
        const lastSection = sections[sections.length - 1];
        lastSection.columns[0].widgets.push(createWidget(widgetType));
        renderCanvas();
    ...
    function createWidget(type) {
        ...
        const defaults = {
            ...
            text:          { content: '<p>Enter your text content here.</p>', alignment: 'left' },
    ...
    function removeSection(sectionId) { sections = sections.filter(s => s.id !== sectionId); closeInspector(); renderCanvas(); }
    function removeWidget(sectionId, colId, widgetId) {
        const section = sections.find(s => s.id === sectionId); if (!section) return;
        const col = section.columns.find(c => c.id === colId); if (!col) return;
        col.widgets = col.widgets.filter(w => w.id !== widgetId); closeInspector(); renderCanvas();
    }
        ...
        if (sections.length === 0) {
            container.innerHTML = `
                <div style="border: 2px dashed var(--cms-border); border-radius: var(--cms-r-lg); padding: 60px 24px; text-align: center; color: var(--cms-fg4); font-family: var(--cms-font-ui), sans-serif; font-size: 13px; display: flex; flex-direction: column; align-items: center; justify-content: center;">
                    ...
                    <div>Use the <strong>Add Section</strong> buttons to start building.</div>
                </div>`;
        ...
        sections.forEach((section, sIdx) => {
            ...
            section.columns.forEach(col => {
                ...
                if (col.widgets.length === 0) {
                    widgetZone.innerHTML = `<div style="text-align: center; padding: 16px 8px; font-family: var(--cms-font-ui), sans-serif; font-size: 11px; color: var(--cms-fg4);">Empty column</div>`;
                } else {
                    col.widgets.forEach(widget => {
                        ...
                        wEl.style.cssText = 'background: var(--cms-surface); border: 1px solid var(--cms-border); border-radius: 6px; padding: 8px 10px; display: flex; align-items: center; justify-content: space-between; transition: border-color 120ms;';
                        ...
                        wEl.innerHTML = `
                            ...
                            <div style="display: flex; gap: 2px; flex-shrink: 0;">
                                <button type="button" onclick="inspectWidget(${section.id}, ${col.id}, ${widget.id})" title="Settings" style="width: 22px; height: 22px; border: none; background: transparent; cursor: pointer; display: flex; align-items: center; justify-content: center; color: var(--cms-gold-deep); border-radius: 4px;"><i data-lucide="settings" style="width: 11px; height: 11px;"></i></button>
                                <button type="button" onclick="removeWidget(${section.id}, ${col.id}, ${widget.id})" title="Remove" style="width: 22px; height: 22px; border: none; background: transparent; cursor: pointer; display: flex; align-items: center; justify-content: center; color: var(--cms-red); border-radius: 4px;"><i data-lucide="x" style="width: 11px; height: 11px;"></i></button>
                            </div>`;
                ...
                addBtn.innerHTML = `<div style="position: relative; display: inline-block;"><button type="button" onclick="toggleColumnMenu(this, ${section.id}, ${col.id})" style="font-family: var(--cms-font-ui), sans-serif; font-size: 10.5px; font-weight: 600; color: var(--cms-fg3); border: 1px dashed var(--cms-border); background: transparent; padding: 5px 12px; border-radius: var(--cms-r-md); cursor: pointer; display: inline-flex; align-items: center; gap: 4px;" onmouseover="this.style.borderColor='var(--cms-gold)'; this.style.color='var(--cms-gold)'" onmouseout="this.style.borderColor='var(--cms-border)'; this.style.color='var(--cms-fg3)'"><i data-lucide="plus" style="width: 11px; height: 11px;"></i> Add Widget</button></div>`;
            ...
            section.columns.forEach(col => {
                ...
                if (zone) {
                    Sortable.create(zone, { animation: 150, handle: '.widget-grip', group: 'widgets', ghostClass: 'pb-sortable-ghost', onEnd: () => syncWidgetOrder() });
                }
            });
        });

        Sortable.create(container, {
            animation: 180, handle: '.section-grip', ghostClass: 'pb-sortable-ghost',
            onEnd: () => {
                const newOrder = [];
                ...
                sections = newOrder; renderCanvas();
            },
        });
        ...
        sections.forEach(section => {
            section.columns.forEach(col => {
                ...
                zone.querySelectorAll('[data-widget-id]').forEach(wEl => {
                    ...
                    let found = null;
                    sections.forEach(s => s.columns.forEach(c => { const w = c.widgets.find(w => w.id === wId); if (w) found = w; }));
    ...
    function toggleColumnMenu(btn, sectionId, colId) {
        ...
        menu.innerHTML = widgetDefs.map(w => `
            <div onclick="addWidgetToColumn(${sectionId}, ${colId}, '${w.type}'); document.querySelectorAll('.pb-widget-menu').forEach(m=>m.remove());"
                ...
                <i data-lucide="${w.icon}" style="width: 13px; height: 13px; color: var(--cms-gold);"></i> ${w.name}
            </div>
        `).join('');
        btn.parentElement.appendChild(menu);
        lucide.createIcons();
        setTimeout(() => { document.addEventListener('click', function handler(e) { if (!menu.contains(e.target) && e.target !== btn) { menu.remove(); document.removeEventListener('click', handler); } }); }, 10);
    }
    ...
    function inspectSection(sectionId) {
        ...
        fields.innerHTML = `
            <div><label class="insp-lbl">Background Theme</label><select id="insp-bg" class="insp-sl">${backgroundPresets.map(b => `<option value="${b.value}" ${section.background===b.value?'selected':''}>${b.label}</option>`).join('')}</select></div>
            <div><label class="insp-lbl">Vertical Padding</label><select id="insp-pad" class="insp-sl">${['small','medium','large'].map(p => `<option value="${p}" ${section.padding===p?'selected':''}>${p.charAt(0).toUpperCase()+p.slice(1)}</option>`).join('')}</select></div>`;
    }

    function inspectWidget(sectionId, colId, widgetId) {
        let widget = null;
        const section = sections.find(s => s.id === sectionId);
        if (section) { const col = section.columns.find(c => c.id === colId); if (col) widget = col.widgets.find(w => w.id === widgetId); }
        if (!widget) return;
        ...
        if (widget.type === 'heading') {
            fields.innerHTML = `<div><label class="insp-lbl">Heading Text</label><input type="text" id="insp-text" value="${escHtml(s.text||'')}" class="insp-in"></div><div><label class="insp-lbl">Level</label><select id="insp-level" class="insp-sl">${['h1','h2','h3','h4','h5','h6'].map(h=>`<option value="${h}" ${s.level===h?'selected':''}>${h.toUpperCase()}</option>`).join('')}</select></div><div><label class="insp-lbl">Alignment</label><select id="insp-align" class="insp-sl">${['left','center','right'].map(a=>`<option value="${a}" ${s.alignment===a?'selected':''}>${a.charAt(0).toUpperCase()+a.slice(1)}</option>`).join('')}</select></div>`;
        } else if (widget.type === 'text') {
            fields.innerHTML = `<div><label class="insp-lbl">Content (HTML)</label><textarea id="insp-content" class="insp-ta" style="min-height: 120px;">${escHtml(s.content||'')}</textarea></div><div><label class="insp-lbl">Alignment</label><select id="insp-align" class="insp-sl">${['left','center','right'].map(a=>`<option value="${a}" ${s.alignment===a?'selected':''}>${a.charAt(0).toUpperCase()+a.slice(1)}</option>`).join('')}</select></div>`;
        } else if (widget.type === 'image') {
            fields.innerHTML = `<div><label class="insp-lbl">Image URL</label><input type="text" id="insp-src" value="${escHtml(s.src||'')}" class="insp-in" placeholder="/storage/media/..."></div><div><label class="insp-lbl">Alt Text</label><input type="text" id="insp-alt" value="${escHtml(s.alt||'')}" class="insp-in"></div><div><label class="insp-lbl">Caption</label><input type="text" id="insp-caption" value="${escHtml(s.caption||'')}" class="insp-in"></div>`;
        } else if (widget.type === 'cta') {
            fields.innerHTML = `<div><label class="insp-lbl">Button Text</label><input type="text" id="insp-text" value="${escHtml(s.text||'')}" class="insp-in"></div><div><label class="insp-lbl">URL Link</label><input type="text" id="insp-url" value="${escHtml(s.url||'')}" class="insp-in"></div><div><label class="insp-lbl">Button Style</label><select id="insp-style" class="insp-sl">${['primary','secondary','dark'].map(v=>`<option value="${v}" ${s.style===v?'selected':''}>${v.charAt(0).toUpperCase()+v.slice(1)}</option>`).join('')}</select></div><div><label class="insp-lbl">Alignment</label><select id="insp-align" class="insp-sl">${['left','center','right'].map(a=>`<option value="${a}" ${s.alignment===a?'selected':''}>${a.charAt(0).toUpperCase()+a.slice(1)}</option>`).join('')}</select></div>`;
        } else if (widget.type === 'video') {
            fields.innerHTML = `<div><label class="insp-lbl">Video URL (YouTube/Vimeo)</label><input type="text" id="insp-url" value="${escHtml(s.url||'')}" class="insp-in"></div>`;
        } else if (widget.type === 'alert') {
            fields.innerHTML = `<div><label class="insp-lbl">Message</label><textarea id="insp-msg" class="insp-ta">${escHtml(s.message||'')}</textarea></div><div><label class="insp-lbl">Alert Type</label><select id="insp-type" class="insp-sl">${['info','warning','success','danger'].map(t=>`<option value="${t}" ${s.type===t?'selected':''}>${t.charAt(0).toUpperCase()+t.slice(1)}</option>`).join('')}</select></div>`;
        } else if (widget.type === 'spacer') {
            fields.innerHTML = `<div><label class="insp-lbl">Height (px)</label><input type="number" id="insp-height" value="${s.height||40}" class="insp-in" min="10" max="200"></div>`;
        } else if (widget.type === 'faq') {
            let h = '<div><label class="insp-lbl">FAQ Items</label></div>';
            (s.items||[]).forEach((item,i) => { h += `<div style="background: var(--cms-bg); padding: 10px; border-radius: 6px; margin-bottom: 4px;"><input type="text" class="insp-in faq-q" data-idx="${i}" value="${escHtml(item.question)}" placeholder="Question" style="margin-bottom: 6px;"><textarea class="insp-ta faq-a" data-idx="${i}" placeholder="Answer">${escHtml(item.answer)}</textarea><button type="button" onclick="removeFaqItem(${i})" style="font-size: 11px; color: var(--cms-red); background: none; border: none; cursor: pointer; margin-top: 4px;">Remove</button></div>`; });
            h += `<button type="button" onclick="addFaqItem()" style="font-family: var(--cms-font-ui), sans-serif; font-size: 11.5px; font-weight: 600; color: var(--cms-gold-deep); background: none; border: 1px dashed var(--cms-border); padding: 6px 12px; border-radius: var(--cms-r-md); cursor: pointer; width: 100%;">+ Add FAQ Item</button>`;
            fields.innerHTML = h;
        } else if (widget.type === 'feature_cards') {
            let h = '<div><label class="insp-lbl">Feature Cards</label></div>';
            (s.cards||[]).forEach((card,i) => { h += `<div style="background: var(--cms-bg); padding: 10px; border-radius: 6px; margin-bottom: 4px;"><div style="display: flex; gap: 6px; margin-bottom: 6px;"><input type="text" class="insp-in card-icon" data-idx="${i}" value="${escHtml(card.icon)}" placeholder="Emoji" style="width: 50px; flex-shrink: 0;"><input type="text" class="insp-in card-title" data-idx="${i}" value="${escHtml(card.title)}" placeholder="Title"></div><textarea class="insp-ta card-desc" data-idx="${i}" placeholder="Description">${escHtml(card.description)}</textarea><button type="button" onclick="removeCard(${i})" style="font-size: 11px; color: var(--cms-red); background: none; border: none; cursor: pointer; margin-top: 4px;">Remove</button></div>`; });
            h += `<button type="button" onclick="addCard()" style="font-family: var(--cms-font-ui), sans-serif; font-size: 11.5px; font-weight: 600; color: var(--cms-gold-deep); background: none; border: 1px dashed var(--cms-border); padding: 6px 12px; border-radius: var(--cms-r-md); cursor: pointer; width: 100%;">+ Add Card</button>`;
            fields.innerHTML = h;
        }
    }

    function showInspector(title, badge) { document.getElementById('inspector-placeholder').style.display = 'none'; document.getElementById('inspector-panel').style.display = 'block'; document.getElementById('inspector-title').textContent = title; document.getElementById('inspector-badge').textContent = badge; }
    function closeInspector() { document.getElementById('inspector-placeholder').style.display = 'block'; document.getElementById('inspector-panel').style.display = 'none'; inspectorTarget = null; }

    function applyInspector() {
        if (!inspectorTarget) return;
        if (inspectorTarget.type === 'section') {
            const section = sections.find(s => s.id === inspectorTarget.sectionId);
            if (section) { section.background = gEl('insp-bg').value; section.padding = gEl('insp-pad').value; }
        } else if (inspectorTarget.type === 'widget') {
            const section = sections.find(s => s.id === inspectorTarget.sectionId); if (!section) return;
            const col = section.columns.find(c => c.id === inspectorTarget.colId); if (!col) return;
            const widget = col.widgets.find(w => w.id === inspectorTarget.widgetId); if (!widget) return;
            const s = widget.settings; const t = widget.type;
            if (t === 'heading') { s.text = gEl('insp-text')?.value?.trim()||''; s.level = gEl('insp-level')?.value||'h2'; s.alignment = gEl('insp-align')?.value||'left'; }
            else if (t === 'text') { s.content = gEl('insp-content')?.value||''; s.alignment = gEl('insp-align')?.value||'left'; }
            else if (t === 'image') { s.src = gEl('insp-src')?.value?.trim()||''; s.alt = gEl('insp-alt')?.value?.trim()||''; s.caption = gEl('insp-caption')?.value?.trim()||''; }
            else if (t === 'cta') { s.text = gEl('insp-text')?.value?.trim()||''; s.url = gEl('insp-url')?.value?.trim()||'#'; s.style = gEl('insp-style')?.value||'primary'; s.alignment = gEl('insp-align')?.value||'center'; }
            else if (t === 'video') { s.url = gEl('insp-url')?.value?.trim()||''; }
            else if (t === 'alert') { s.message = gEl('insp-msg')?.value?.trim()||''; s.type = gEl('insp-type')?.value||'info'; }
            else if (t === 'spacer') { s.height = parseInt(gEl('insp-height')?.value)||40; }
            else if (t === 'faq') { const items = []; document.querySelectorAll('.faq-q').forEach(qEl => { const idx = parseInt(qEl.dataset.idx); const aEl = document.querySelector(`.faq-a[data-idx="${idx}"]`); items.push({ question: qEl.value.trim(), answer: aEl?.value?.trim()||'' }); }); s.items = items; }
            else if (t === 'feature_cards') { const cards = []; document.querySelectorAll('.card-icon').forEach(iconEl => { const idx = parseInt(iconEl.dataset.idx); const titleEl = document.querySelector(`.card-title[data-idx="${idx}"]`); const descEl = document.querySelector(`.card-desc[data-idx="${idx}"]`); cards.push({ icon: iconEl.value.trim(), title: titleEl?.value?.trim()||'', description: descEl?.value?.trim()||'' }); }); s.cards = cards; }
        }
        renderCanvas(); closeInspector(); showToast('Settings applied.');
    }

    function addFaqItem() { if (!inspectorTarget?.widgetId) return; const s = sections.find(s=>s.id===inspectorTarget.sectionId); const c = s?.columns.find(c=>c.id===inspectorTarget.colId); const w = c?.widgets.find(w=>w.id===inspectorTarget.widgetId); if (w?.type==='faq') { w.settings.items.push({question:'',answer:''}); inspectWidget(inspectorTarget.sectionId, inspectorTarget.colId, inspectorTarget.widgetId); } }
    function removeFaqItem(idx) { if (!inspectorTarget?.widgetId) return; const s = sections.find(s=>s.id===inspectorTarget.sectionId); const c = s?.columns.find(c=>c.id===inspectorTarget.colId); const w = c?.widgets.find(w=>w.id===inspectorTarget.widgetId); if (w?.type==='faq') { w.settings.items.splice(idx,1); inspectWidget(inspectorTarget.sectionId, inspectorTarget.colId, inspectorTarget.widgetId); } }
    function addCard() { if (!inspectorTarget?.widgetId) return; const s = sections.find(s=>s.id===inspectorTarget.sectionId); const c = s?.columns.find(c=>c.id===inspectorTarget.colId); const w = c?.widgets.find(w=>w.id===inspectorTarget.widgetId); if (w?.type==='feature_cards') { w.settings.cards.push({icon:'⭐',title:'',description:''}); inspectWidget(inspectorTarget.sectionId, inspectorTarget.colId, inspectorTarget.widgetId); } }
    function removeCard(idx) { if (!inspectorTarget?.widgetId) return; const s = sections.find(s=>s.id===inspectorTarget.sectionId); const c = s?.columns.find(c=>c.id===inspectorTarget.colId); const w = c?.widgets.find(w=>w.id===inspectorTarget.widgetId); if (w?.type==='feature_cards') { w.settings.cards.splice(idx,1); inspectWidget(inspectorTarget.sectionId, inspectorTarget.colId, inspectorTarget.widgetId); } }
