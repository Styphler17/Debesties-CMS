...
<style>
    ...
    .seo-score-text--high { color: var(--cms-green); }
    .seo-score-text--mid  { color: var(--cms-gold); }
    .seo-score-text--low  { color: var(--cms-red); }

    .seo-score-bar { height: 100%; border-radius: 999px; }
    .seo-score-bar--high { background: var(--cms-green); }
    .seo-score-bar--mid  { background: var(--cms-gold); }
    .seo-score-bar--low  { background: var(--cms-red); }
</style>
...
.issue-count {
    font-family: var(--cms-font-disp), serif;
    font-size: 32px;
    font-weight: 700;
}
    ...
    <div style="display: grid; grid-template-columns: 200px 1fr; gap: 20px; align-items: start;">
        ...
        <div style="background: var(--cms-surface); border: 1px solid var(--cms-border); border-radius: var(--cms-r-lg); padding: 24px 20px; text-align: center; box-shadow: var(--cms-sh-card);">
            <svg viewBox="0 0 120 80" width="120" height="80" style="display: block; margin: 0 auto 10px;">
                ...
                @php
                    ...
                    $pathStyle = "stroke: $gaugeColor; stroke-dasharray: $filled $circumference;";
                @endphp
                <path d="M15 70 A55 55 0 0 1 105 70" fill="none"
                      stroke-width="10" stroke-linecap="round"
                      class="seo-gauge-path"
                      @style([
                        "stroke: $gaugeColor",
            ...
            <div style="font-family: var(--cms-font-ui), sans-serif; font-size: 13px; font-weight: 700; color: var(--cms-fg1);">SEO Health Score</div>
            <div style="font-family: var(--cms-font-ui), sans-serif; font-size: 11.5px; color: var(--cms-fg4); margin-top: 3px;">avg. across {{ count($auditPosts) }} posts</div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 14px;">
            ...
            @foreach($issues as $issue)
                <div class="seo-issue-card"
                     @style([
                        "--icon-bg: {$issue['bg']}",
                        "--icon-color: {$issue['color']}",
                     ])
                    ...
                    <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
                        ...
                        <span style="font-family: var(--cms-font-ui), sans-serif; font-size: 13px; color: var(--cms-fg2);">{{ $issue['label'] }}</span>
                    </div>
                    <div class="issue-count {{ $issue['count'] > 0 ? 'issue-count--bad' : 'issue-count--good' }}">{{ $issue['count'] }}</div>
                    <div style="font-family: var(--cms-font-ui), sans-serif; font-size: 12px; color: var(--cms-fg4); margin-top: 2px;">{{ $issue['count'] > 0 ? 'need attention' : 'all good' }}</div>
                </div>
            @endforeach
        </div>
    </div>
...
<div id="tab-audit" style="display: none;">
    <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 14px; flex-wrap: wrap;">
        <span style="font-family: var(--cms-font-ui), sans-serif; font-size: 13px; color: var(--cms-fg3);">Filter:</span>
                ...
                @foreach($auditPosts as $p)
                    ...
                    <tr class="audit-row"
                        ...
                        <td style="padding: 12px 16px 12px 0; max-width: 160px;">
                            @if($p['meta_title'])
                                <span style="font-family: var(--cms-font-ui), sans-serif; font-size: 12.5px; color: var(--cms-fg2); display: block; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $p['meta_title'] }}</span>
                            @else
                                <span style="font-family: var(--cms-font-ui), sans-serif; font-size: 12px; color: var(--cms-red); display: flex; align-items: center; gap: 4px;"><i data-lucide="alert-circle" style="width:12px;height:12px;"></i> Missing</span>
                            @endif
                        </td>
                        <td style="padding: 12px 16px 12px 0; max-width: 160px;">
                            @if($p['meta_desc'])
                                <span style="font-family: var(--cms-font-ui), sans-serif; font-size: 12.5px; color: var(--cms-fg2); display: block; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $p['meta_desc'] }}</span>
                            @else
                                <span style="font-family: var(--cms-font-ui), sans-serif; font-size: 12px; color: var(--cms-red); display: flex; align-items: center; gap: 4px;"><i data-lucide="alert-circle" style="width:12px;height:12px;"></i> Missing</span>
                            @endif
                        </td>
                        <td style="padding: 12px 16px 12px 0;">
                            @if($p['keyword'])
                                <span style="font-family: var(--cms-font-ui), sans-serif; font-size: 12.5px; color: var(--cms-fg2);">{{ $p['keyword'] }}</span>
                            @else
                                <span style="font-family: var(--cms-font-ui), sans-serif; font-size: 12px; color: var(--cms-fg4);">—</span>
                            @endif
                        </td>
                        <td style="padding: 12px 16px 12px 0; white-space: nowrap;">
                            @php
                                $barStyle = "width: {$p['score']}%";
                            @endphp
                            <div style="display: flex; align-items: center; gap: 8px;">
                                <span @class(["seo-score-text--$scoreLevel"]) style="font-family: var(--cms-font-ui), sans-serif; font-size: 13px; font-weight: 700; min-width: 26px;">{{ $p['score'] }}</span>
                                <div style="width: 50px; height: 5px; background: var(--cms-border); border-radius: 999px; overflow: hidden;">
                                    <div @class(['seo-score-bar', "seo-score-bar--$scoreLevel"]) 
                        ...
                        <td style="padding: 12px 16px;">
                            <a href="{{ route('admin.posts.edit', $p['id']) }}"
                               style="height: 30px; padding: 0 12px; display: inline-flex; align-items: center; gap: 5px; font-family: var(--cms-font-ui), sans-serif; font-size: 12.5px; font-weight: 600; color: var(--cms-blue); background: rgba(74,121,255,0.08); border-radius: var(--cms-r-md); text-decoration: none;"
...
<div id="tab-links" style="display: none;">
    <div style="background: var(--cms-surface); border: 1px solid var(--cms-border); border-radius: var(--cms-r-lg); overflow: hidden; box-shadow: var(--cms-sh-card);">
        <div style="padding: 14px 20px; border-bottom: 1px solid var(--cms-border);">
            <span style="font-family: var(--cms-font-ui), sans-serif; font-size: 14px; font-weight: 700; color: var(--cms-fg1);">Internal Link Coverage</span>
            <span style="font-family: var(--cms-font-ui), sans-serif; font-size: 12.5px; color: var(--cms-fg4); margin-left: 8px;">via SuggestInternalLinks action</span>
        </div>
        <table style="width: 100%; border-collapse: collapse;">
            <thead>
                <tr style="border-bottom: 1px solid var(--cms-border);">
                ...
                @foreach($internalLinks as $lp)
                    <tr style="border-bottom: 1px solid var(--cms-border); transition: background 100ms;"
                        ...
                        <td style="padding: 13px 0 13px 16px;">
                            <span style="font-family: var(--cms-font-ui), sans-serif; font-size: 13.5px; font-weight: 600; color: var(--cms-fg1);">{{ $lp['title'] }}</span>
                        </td>
                        <td style="padding: 13px 16px 13px 0;">
                            <span class="link-count {{ $lp['link_count'] === 0 ? 'link-count--zero' : 'link-count--normal' }}"
                                  style="font-family: var(--cms-font-ui), sans-serif; font-size: 13px; font-weight: 600;">{{ $lp['link_count'] }}</span>
                            <span style="font-family: var(--cms-font-ui), sans-serif; font-size: 12px; color: var(--cms-fg4);"> links</span>
                        </td>
                        <td style="padding: 13px 16px 13px 0;">
                            @if($lp['suggestions'] > 0)
                                <span style="font-family: var(--cms-font-ui), sans-serif; font-size: 12.5px; font-weight: 600; color: var(--cms-gold); background: var(--cms-gold-soft); padding: 2px 8px; border-radius: 999px;">{{ $lp['suggestions'] }} suggested</span>
                            @else
                        ...
                        <td style="padding: 13px 16px;">
                            <a href="{{ route('admin.posts.edit', $lp['id']) }}"
                               style="height: 30px; padding: 0 12px; display: inline-flex; align-items: center; font-family: var(--cms-font-ui), sans-serif; font-size: 12.5px; font-weight: 600; color: var(--cms-blue); background: rgba(74,121,255,0.08); border-radius: var(--cms-r-md); text-decoration: none;"
                               onmouseover="this.style.background='rgba(74,121,255,0.15)'" onmouseout="this.style.background='rgba(74,121,255,0.08)'">
                                Edit
                            </a>
                        </td>
