...
<div style="max-width: 900px; margin: 0 auto; display: flex; flex-direction: column; gap: 24px;">
        ...
        @if($newsletter->status === 'draft')
            <div style="display: flex; gap: 10px;">
                <a href="{{ route('admin.newsletters.edit', $newsletter) }}" 
                   style="display: inline-flex; align-items: center; gap: 6px; font-family: var(--cms-font-ui); font-size: 13px; font-weight: 600; padding: 8px 14px; background: var(--cms-surface); border: 1.5px solid var(--cms-border); border-radius: var(--cms-r-md); color: var(--cms-fg1); text-decoration: none; cursor: pointer;">
                ...
                <form action="{{ route('admin.newsletters.destroy', $newsletter) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this draft campaign?')" style="display: inline;">
                    ...
                    @method('DELETE')
                    <button type="submit" 
                            style="display: inline-flex; align-items: center; gap: 6px; font-family: var(--cms-font-ui); font-size: 13px; font-weight: 600; padding: 8px 14px; background: var(--cms-surface); border: 1.5px solid var(--cms-red); border-radius: var(--cms-r-md); color: var(--cms-red); cursor: pointer;">
                ...
                <form action="{{ route('admin.newsletters.send', $newsletter) }}" method="POST" onsubmit="return confirm('Are you sure you want to broadcast this newsletter to all subscribers? This action cannot be undone.')" style="display: inline;">
                    @csrf
                    <button type="submit" 
                            style="display: inline-flex; align-items: center; gap: 6px; font-family: var(--cms-font-ui); font-size: 13px; font-weight: 700; padding: 8px 16px; background: var(--cms-green-deep); border: none; border-radius: var(--cms-r-md); color: #fff; cursor: pointer; transition: background 120ms;"
    ...
    <div style="background: var(--cms-surface); border: 1px solid var(--cms-border); border-radius: var(--cms-r-lg); overflow: hidden; box-shadow: var(--cms-sh-card); padding: 24px; display: grid; grid-template-columns: 2fr 1fr; gap: 24px;">
        ...
        <div>
            <span style="font-family: var(--cms-font-ui); font-size: 11px; font-weight: 700; color: var(--cms-fg3); text-transform: uppercase; letter-spacing: 0.05em; display: block; margin-bottom: 4px;">Subject Line</span>
            <h2 style="font-family: var(--cms-font-ui); font-size: 18px; font-weight: 800; color: var(--cms-fg1); margin: 0 0 16px 0;">{{ $newsletter->subject }}</h2>

            @if($newsletter->status === 'sent')
                <div style="background: var(--cms-green-soft); border: 1.5px solid var(--cms-green-deep); border-radius: var(--cms-r-md); padding: 14px 16px; display: flex; align-items: flex-start; gap: 10px; color: var(--cms-green-deep); font-family: var(--cms-font-ui); font-size: 13.5px;">
            ...
            @elseif($newsletter->status === 'sending')
                <div style="background: var(--cms-blue-soft); border: 1.5px solid var(--cms-blue); border-radius: var(--cms-r-md); padding: 14px 16px; display: flex; align-items: flex-start; gap: 10px; color: var(--cms-blue); font-family: var(--cms-font-ui); font-size: 13.5px;">
            ...
            @else
                <div style="background: #F0EDE8; border: 1.5px solid var(--cms-border-st); border-radius: var(--cms-r-md); padding: 14px 16px; display: flex; align-items: flex-start; gap: 10px; color: var(--cms-fg2); font-family: var(--cms-font-ui); font-size: 13.5px;">
        ...
        <div style="border-left: 1px solid var(--cms-border); padding-left: 24px; display: flex; flex-direction: column; gap: 14px; font-family: var(--cms-font-ui);">
            <div>
                ...
                @endphp
                <span style="display: inline-flex; align-items: center; height: 22px; padding: 0 8px; border-radius: 4px; font-size: 10.5px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.02em; background: {{ $statusBg }}; color: {{ $statusColor }};">
                    {{ $newsletter->status }}
                </span>
            </div>
    ...
    <div>
        <h4 style="font-family: var(--cms-font-ui); font-size: 13px; font-weight: 700; color: var(--cms-fg3); text-transform: uppercase; letter-spacing: 0.04em; margin: 0 0 10px 0;">Email Layout Preview</h4>
