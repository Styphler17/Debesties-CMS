...
<style>
    .media-row { transition: background 100ms; cursor: pointer; }
    .media-row--border { border-bottom: 1px solid var(--cms-border); }
</style>
...
<div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap; margin-bottom: 16px;">
    <form method="GET" action="{{ route('admin.media.index') }}" style="display: flex; align-items: center; gap: 10px; flex: 1; flex-wrap: wrap;">
        <div style="display: flex; align-items: center; gap: 8px; flex: 1; min-width: 200px; height: 38px; background: var(--cms-surface); border: 1.5px solid var(--cms-border); border-radius: var(--cms-r-md); padding: 0 12px;">
            ...
            <input name="search" value="{{ request('search') }}" placeholder="Search files…" style="border: none; outline: none; background: none; flex: 1; font-family: var(--cms-font-ui), sans-serif; font-size: 13.5px; color: var(--cms-fg1);" />
        </div>
        <select name="folder" onchange="this.form.submit()"
                style="height: 38px; padding: 0 12px; font-family: var(--cms-font-ui), sans-serif; font-size: 13.5px; color: var(--cms-fg1); background: var(--cms-surface); border: 1.5px solid var(--cms-border); border-radius: var(--cms-r-md); cursor: pointer; outline: none;">
    ...
    <button onclick="document.getElementById('file-upload-input').click()"
            style="display: inline-flex; align-items: center; gap: 7px; height: 38px; padding: 0 18px; font-family: var(--cms-font-ui), sans-serif; font-size: 13.5px; font-weight: 600; background: var(--cms-gold); color: #1A1410; border: none; border-radius: var(--cms-r-md); cursor: pointer;"
    ...
    @forelse($files as $file)
        <div class="media-card" data-name="{{ strtolower($file->file_name) }}" data-folder="{{ $file->folder }}"
            ...
            <div style="padding: 10px 12px;">
                <div style="font-family: var(--cms-font-ui), sans-serif; font-size: 12.5px; font-weight: 600; color: var(--cms-fg1); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $file->file_name }}</div>
                <div style="display: flex; align-items: center; justify-content: space-between; margin-top: 4px;">
                    <span style="font-family: var(--cms-font-ui), sans-serif; font-size: 11.5px; color: var(--cms-fg4);">{{ number_format($file->file_size / 1024, 0) }} KB</span>
                    <span style="font-family: var(--cms-font-ui), sans-serif; font-size: 11px; color: var(--cms-fg4);">{{ $file->width }}×{{ $file->height }}</span>
                </div>
            </div>
        </div>
    @empty
        <div style="grid-column: 1/-1; padding: 48px; text-align: center; background: var(--cms-surface); border: 1px dashed var(--cms-border); border-radius: var(--cms-r-lg);">
            ...
            <div style="font-family: var(--cms-font-ui), sans-serif; font-size: 15px; font-weight: 600; color: var(--cms-fg2);">No media files found</div>
        </div>
    @endforelse
</div>
    ...
    <table style="width: 100%; border-collapse: collapse;">
        <thead>
            <tr style="border-bottom: 1px solid var(--cms-border);">
                ...
                <th style="padding: 10px 16px 10px 0; text-align: left; font-size: 12px; font-weight: 700; color: var(--cms-fg3); text-transform: uppercase; letter-spacing: 0.04em; font-family: var(--cms-font-ui), sans-serif;">Name</th>
                <th style="padding: 10px 16px 10px 0; text-align: left; font-size: 12px; font-weight: 700; color: var(--cms-fg3); text-transform: uppercase; letter-spacing: 0.04em; font-family: var(--cms-font-ui), sans-serif;">Type</th>
                <th style="padding: 10px 16px 10px 0; text-align: right; font-size: 12px; font-weight: 700; color: var(--cms-fg3); text-transform: uppercase; letter-spacing: 0.04em; font-family: var(--cms-font-ui), sans-serif;">Size</th>
                <th style="padding: 10px 16px 10px 0; text-align: center; font-size: 12px; font-weight: 700; color: var(--cms-fg3); text-transform: uppercase; letter-spacing: 0.04em; font-family: var(--cms-font-ui), sans-serif;">Dimensions</th>
                <th style="padding: 10px 16px 10px 0; text-align: right; font-size: 12px; font-weight: 700; color: var(--cms-fg3); text-transform: uppercase; letter-spacing: 0.04em; font-family: var(--cms-font-ui), sans-serif;">Uploaded</th>
                <th style="padding: 10px 16px; width: 60px;"></th>
            </tr>
        </thead>
        <tbody>
            @forelse($files as $i => $file)
                <tr @class(['media-row', 'media-row--border' => ! $loop->last])
                    onclick="openDetail({{ $file->id }})"
                    ...
                    <td style="padding: 11px 16px 11px 0;">
                        <span style="font-family: var(--cms-font-ui), sans-serif; font-size: 13.5px; font-weight: 500; color: var(--cms-fg1);">{{ $file->file_name }}</span>
                    </td>
                    <td style="padding: 11px 16px 11px 0;">
                        <span style="font-family: var(--cms-font-mono), monospace; font-size: 11.5px; color: var(--cms-fg3);">{{ $file->mime_type }}</span>
                    </td>
                    <td style="padding: 11px 16px 11px 0; text-align: right;">
                        <span style="font-family: var(--cms-font-ui), sans-serif; font-size: 12.5px; color: var(--cms-fg3);">{{ number_format($file->file_size / 1024, 0) }} KB</span>
                    </td>
                    <td style="padding: 11px 16px 11px 0; text-align: center;">
                        <span style="font-family: var(--cms-font-mono), monospace; font-size: 11.5px; color: var(--cms-fg3);">{{ $file->width }}×{{ $file->height }}</span>
                    </td>
                    <td style="padding: 11px 16px 11px 0; text-align: right;">
                        <span style="font-family: var(--cms-font-ui), sans-serif; font-size: 12px; color: var(--cms-fg4);">{{ $file->created_at->diffForHumans() }}</span>
                    </td>
            ...
            @empty
                <tr>
                    <td colspan="7" style="padding: 32px; text-align: center; font-family: var(--cms-font-ui), sans-serif; font-size: 14px; color: var(--cms-fg3);">No media files found</td>
                </tr>
            @endforelse
        </tbody>
    </table>
...
<div id="detail-panel"
    ...
    <div style="display: flex; align-items: center; justify-content: space-between; padding: 16px 18px; border-bottom: 1px solid var(--cms-border); position: sticky; top: 0; background: var(--cms-surface); z-index: 2;">
        <span style="font-family: var(--cms-font-ui), sans-serif; font-size: 14px; font-weight: 700; color: var(--cms-fg1);">File Details</span>
    ...
    <div style="padding: 18px; flex: 1;">
        ...
        <div style="display: flex; flex-direction: column; gap: 12px;">
            <div>
                <label style="font-family: var(--cms-font-ui), sans-serif; font-size: 11px; font-weight: 700; color: var(--cms-fg4); text-transform: uppercase; letter-spacing: 0.06em; display: block; margin-bottom: 5px;">Alt Text</label>
                <input id="detail-alt" type="text" readonly style="display: block; width: 100%; height: 38px; padding: 0 10px; font-family: var(--cms-font-ui), sans-serif; font-size: 13px; color: var(--cms-fg1); background: var(--cms-bg); border: 1.5px solid var(--cms-border); border-radius: var(--cms-r-md); outline: none;" />
            </div>
            <div>
                <label style="font-family: var(--cms-font-ui), sans-serif; font-size: 11px; font-weight: 700; color: var(--cms-fg4); text-transform: uppercase; letter-spacing: 0.06em; display: block; margin-bottom: 5px;">File URL</label>
                <div style="display: flex; gap: 6px;">
                    <input id="detail-url" readonly style="flex: 1; height: 34px; padding: 0 10px; font-family: var(--cms-font-mono), monospace; font-size: 11.5px; color: var(--cms-fg3); background: var(--cms-bg); border: 1.5px solid var(--cms-border); border-radius: var(--cms-r-md); outline: none;" />
    ...
    <div style="padding: 16px 18px; border-top: 1px solid var(--cms-border); display: flex; gap: 8px; position: sticky; bottom: 0; background: var(--cms-surface);">
        <form id="detail-delete-form" method="POST" action="" style="flex: 1;" onsubmit="return confirm('Are you sure you want to delete this file?')">
            @csrf
            @method('DELETE')
            <button type="submit" style="width: 100%; height: 38px; font-family: var(--cms-font-ui), sans-serif; font-size: 13px; font-weight: 600; background: var(--cms-red-soft); color: var(--cms-red-deep); border: 1.5px solid rgba(200,55,43,0.2); border-radius: var(--cms-r-md); cursor: pointer;">Delete File</button>
        </form>
    </div>
</div>

<script id="media-data" type="application/json">
    {!! json_encode($files->items()) !!}
</script>
