...
@section('title', 'Category: ' . $category->name . ' - ' . \App\Services\SettingsService::get('site_name', 'Debesties CMS'))
...
@section('content')
            ...
            @foreach($posts as $post)
                <article class="article-card">
                    ...
                    <div class="card-content">
                        <span class="kicker" style="color: var(--accent-green);">{{ $category->name }}</span>
                        <h4 class="card-title">
    ...
    @else
        <div style="text-align: center; padding: 4rem 1.5rem; border: 1px dashed var(--border-color); border-radius: var(--radius-lg); background-color: var(--card-bg);">
            <p style="color: var(--text-muted); font-size: 1.1rem;">No articles found in this category yet.</p>
        </div>
    @endif
