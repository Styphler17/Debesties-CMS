    ...
    private function resizeImage(string $src, string $dst, int $targetWidth, ?int $targetHeight, bool $crop): void
    {
        ...
        switch ($type) {
            ...
            case IMAGETYPE_GIF:  $img = imagecreatefromgif($src);
                break;
            case IMAGETYPE_WEBP: $img = imagecreatefromwebp($src);
                break;
            default: return;
        }
        ...
        switch ($type) {
            case IMAGETYPE_JPEG: imagejpeg($resized, $dst, 85);
                break;
            case IMAGETYPE_PNG:  imagepng($resized, $dst, 6);
                break;
            case IMAGETYPE_GIF:  imagegif($resized, $dst);
                break;
            case IMAGETYPE_WEBP: imagewebp($resized, $dst, 80);
                break;
        }
